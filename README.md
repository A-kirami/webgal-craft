<p align="center">
  <a href="https://github.com/A-kirami/webgal-craft">
    <br />
    <img src="./public/webgal-craft-logo.svg" alt="WebGAL Craft Logo" width="160" />
    <br />
    <h1 align="center">WebGAL Craft</h1>
  </a>
</p>

<p align="center">
  面向 WebGAL 创作者的跨平台视觉小说工作室
</p>

<p align="center">
  <a href="https://github.com/A-kirami/webgal-craft/actions/workflows/build.yml" target="__blank"
    ><img src="https://github.com/A-kirami/webgal-craft/actions/workflows/build.yml/badge.svg?branch=main&event=push" alt="Github Actions"
  /></a>
  <a href="https://github.com/A-kirami/webgal-craft/actions/workflows/test.yml" target="__blank"
    ><img src="https://github.com/A-kirami/webgal-craft/actions/workflows/test.yml/badge.svg?branch=main&event=push" alt="Github Actions"
  /></a>
  <a href="./LICENSE">
    <img src="https://img.shields.io/badge/license-MPL--2.0-blue" alt="License MPL-2.0" />
  </a>
  <img src="https://img.shields.io/badge/platform-Windows%20%7C%20macOS%20%7C%20Linux-6f42c1" alt="Supported platforms: Windows, macOS, Linux" />
  <br />
  <a href="https://github.com/A-kirami/webgal-craft/releases" target="__blank"
    ><img src="https://img.shields.io/github/v/release/A-kirami/webgal-craft?include_prereleases&&color=70aeff&style=social" alt="Release Version"
  /></a>
  <a href="https://github.com/A-kirami/webgal-craft/stargazers" target="__blank"
    ><img alt="GitHub stars" src="https://img.shields.io/github/stars/A-kirami/webgal-craft?style=social"
  /></a>
  <a href="https://github.com/A-kirami/webgal-craft/releases" target="__blank"
    ><img alt="GitHub downloads" src="https://img.shields.io/github/downloads/A-kirami/webgal-craft/total?style=social"
  /></a>
</p>

<p align="center">
  <a href="https://github.com/A-kirami/webgal-craft/releases">下载最新版</a>
  ·
  <a href="https://webgalcraft.com">官方网站</a>
  ·
  <a href="https://github.com/A-kirami/webgal-craft/issues">反馈问题</a>
  ·
  <a href="https://qm.qq.com/q/W2Dt9V2F66">加入交流群</a>
</p>

WebGAL Craft 是一款为 [WebGAL](https://github.com/OpenWebGAL/WebGAL) 游戏创作者设计的创作工具。它把项目管理、脚本编辑、资源浏览、可视化语句编辑和实时预览放在同一个工作流里，让创作者可以更直接地组织素材、编写演出并检查游戏效果。

## 特性

**资源管理**

集中管理立绘、背景、音效等项目素材，在创作过程中随取随用。

**可视化编辑**

在可视化界面中组织剧情、对白和演出参数，同时保留脚本编辑能力，兼顾上手效率与精细控制。

**本地历史版本**

自动记录场景保存历史，脚本变更可追溯、可回退，长篇项目也能更稳妥地维护。

**快速预览**

在编辑器中实时预览演出效果，边创作边验证，减少在编辑器和游戏窗口之间反复切换。

**引擎与模板切换**

按作品需要管理和切换 WebGAL 引擎与项目模板，便于适配不同版本和制作风格。

**演出参数复用**

保存常用语句默认值与语句组，减少重复配置和手写脚本，让创作流程更稳定。

## 下载安装

前往 [Releases](https://github.com/A-kirami/webgal-craft/releases) 下载适合你系统的最新版本。

| 平台 | 构建 | 推荐下载 |
| --- | --- | --- |
| Windows | 64-bit | `.exe` |
| macOS | Intel / Apple Silicon | `.dmg` |
| Linux | x64 | `.AppImage` / `.deb` |

## 快速上手

> [!IMPORTANT]
> WebGAL Craft 需要 WebGAL `4.6.2` 或以上版本的引擎，可在 [WebGAL Releases](https://github.com/OpenWebGAL/WebGAL/releases) 获取

1. 下载并安装适合你系统的 WebGAL Craft。
2. 打开 WebGAL Craft，在主页安装游戏引擎。
3. 创建新游戏，或导入已有 WebGAL 游戏目录。
4. 进入编辑后，在文本/可视化编辑器、资源面板和预览面板之间完成创作。

## 贡献

WebGAL Craft 欢迎任何形式的贡献，详情请阅读[贡献指南](./.github/CONTRIBUTING.md)。

## 许可证

Code: [MPL-2.0](./LICENSE), 2026, Akirami

Logo: CC-BY-NC-ND, Designs by Akirami

## 鸣谢

### 贡献者

感谢以下贡献者对 WebGAL Craft 作出的贡献

<a href="https://github.com/A-kirami/webgal-craft/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=A-kirami/webgal-craft" />
</a>
